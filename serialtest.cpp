...
/* * * * * * * * * * *  * * * * * * * * * * * * * * * * *
lin_function parses the equation, lower limit and upper limit
 obtained and calculates the area under the graph
...
/* * * * * * * * * * *  * * * * * * * * * * * * * * * * *
graph_evaluator parses the equation, plugs in a set of test
points and stores the result in a txt file
...
void graph_evaluator()
{
   ...
   { 
      ...
      // sends the maxiumum and minimum
...
/* * * * * * * * * * *  * * * * * * * * * * * * * * * * *
This fucntion is executed as soon as the file is run

It also does not return any parameters

* * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
int main() {
  // Declaration of serialport for client communication
  SerialPort Serial("/dev/ttyACM0");

  // Declaration of different stages of communication
  ...
  while (true){
    ...
    if (line.find("LowerL:") != std::string::npos) {
      ...
      rec = Serial.readline();
      if(rec == "Ack\n") {
        // Waits for acknowlegdement to read and send the test points to the client
    ...
    if (line.find("UpperL:") != std::string::npos) {
      ...
      // Writing the area into the client
