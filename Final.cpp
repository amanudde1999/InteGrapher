...
// Global variable strings of the function, the lower limit and the upper limit to be sent
...
void mainMenu(){
    ...
    tft.fillRect(118,158,244,64,BLACK);
    ...
    // Function to draw the title screen image
...
/* * * * * * * * * * *  * * * * * * * * * * * * * * * * *
setMaxMin function takes no parameters.

It also does not return any parameters

This will parse maximum and minimum test point value for graph scale
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
/* * * * * * * * * * *  * * * * * * * * * * * * * * * * *
drawGraph function takes no parameters.

It also does not return any parameters

This will parse the point string and draw the obtained graph on the screen
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
void drawGraph() {
    String line = buffer;
    line = line.substring(2);
    ...
    tft.drawCircle(mapped_x,mapped_y,1,BLUE);
    tft.setCursor(170,10);
    tft.print(x_max/1000);
    tft.setCursor(170,310);
    if (x_min < 0){
        tft.print(x_min/1000);
    }
    else{
        tft.print("-"+(x_min/1000));
    }
...
/* * * * * * * * * * *  * * * * * * * * * * * * * * * * *
process_line function takes no parameters.

It also does not return any parameters

This function processes the obtained string buffer obtained and 
...
void process_line() {
    ...
    // If we are recieving a point
    if ((buffer[0] == 'P')) {
        drawGraph();
    }
    // If we are recieving a min/max value
    if (buffer[0] == 'M') {
        setMaxMin();
    }    
...
This will setup the graph display and plot the graph
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
void graphDisplay(){
    ...
    tft.setTextColor(BLACK);
    tft.print("QUIT");
    tft.fillRect(358,158,54,154,BLACK);
    tft.fillRect(418,158,54,154,BLACK);
    tft.fillRect(360,160,50,150,GRAY);
    tft.fillRect(420,160,50,150,GRAY);
    ...
    tft.println("O");
    tft.setCursor(380,220);
    tft.println("O");
    tft.setCursor(380,240);
    tft.println("M");
    tft.setCursor(385,270);
    tft.println("I");
    tft.setCursor(380,290);
    tft.println("N");
    tft.setCursor(440,180);
    tft.println("Z");
    tft.setCursor(440,200);
    tft.println("O");
    tft.setCursor(440,220);
    tft.println("O");
    tft.setCursor(440,240);
    tft.println("M");
    tft.setCursor(440,265);
    tft.println("O");
    tft.setCursor(440,285);
    tft.println("U");
    tft.setCursor(440,305);
    tft.println("T");
...
Allows the user to enter the upper and lower limits of
the integral.
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * */ 
...
void limitEntry(){
    ...
    tft.drawFastHLine(5,170,380,WHITE);
    tft.drawFastHLine(5,220,380,WHITE);
    tft.drawFastHLine(5,270,380,WHITE);
    tft.drawFastHLine(5,318,470,WHITE);
    tft.drawFastHLine(385,180,98,WHITE);
    tft.drawFastHLine(385,250,98,WHITE);
    tft.drawFastHLine(385,320,98,WHITE);
    ...
    // Replacement for integration symbol
    tft.drawLine(40,10,20,110,BLACK);

    // COLUMN 1
    tft.fillRect(10,125,80,40,WHITE);
    tft.setCursor(40,160);
    tft.print("0");
    tft.fillRect(10,175,80,40,WHITE);
    tft.setCursor(40,205);
    tft.print("4");
    tft.fillRect(10,225,80,40,WHITE);
    tft.setCursor(40,255);
    tft.print("8");
    tft.fillRect(10,275,80,40,WHITE);
    tft.setCursor(40,305);
    tft.print("+");
    // COLUMN 2
    tft.fillRect(100,125,85,40,WHITE);
    tft.setCursor(135,160);
    tft.print("1");
    tft.fillRect(100,175,85,40,WHITE);
    tft.setCursor(135,205);
    tft.print("5");
    tft.fillRect(100,225,85,40,WHITE);
    tft.setCursor(135,255);
    tft.print("9");
    tft.fillRect(100,275,85,40,WHITE);
    tft.setCursor(135,290);
    tft.print("_");
    // COLUMN 3
    tft.fillRect(200,125,85,40,WHITE);
    tft.setCursor(230,160);
    tft.print("2");
    tft.fillRect(200,175,85,40,WHITE);
    tft.setCursor(230,205);
    tft.print("6");
    tft.fillRect(200,225,85,40,WHITE);
    tft.setCursor(230,255);
    tft.print(".");
    tft.fillRect(200,275,85,40,WHITE);
    tft.setCursor(230,305);
    tft.print("*");
    // COLUMN 4
    tft.fillRect(295,125,85,40,WHITE);
    tft.setCursor(325,160);
    tft.print("3");
    tft.fillRect(295,175,85,40,WHITE);
    tft.setCursor(325,205);
    tft.print("7");
    tft.fillRect(295,225,85,40,WHITE);
    tft.setCursor(325,255);
    tft.print("pi");
    tft.fillRect(295,275,85,40,WHITE);
    tft.setCursor(325,305);
    tft.print("/");
    // COLUMN 5
    tft.fillRect(390,125,80,50,RED);
    tft.setCursor(395,160);
    tft.print("CLR");
    ...
    // Go and Up button arent pressed therefore flag = 0
    int go2_flag = 0;
    int next_flag = 0;
    tft.setTextSize(1);
    ...
    while(go2_flag != 1){
        ...
        if(touch.z > MINPRESSURE && touch.z < MAXPRESSURE){
            ...
            if (point_x > 10 && point_x < 90 && point_y > 255 && point_y < 315){
                // This will allows us to go the the limit entry mode
                go2_flag = 1;
                // Command used to send equation to the server
                Serial.print("Equation: ");
                Serial.println(Functionstring);
            }
            else if (point_x > 390 && point_x < 470 && point_y > 125 && point_y <165){
                    tft.print("0");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 390 && point_x < 470 && point_y > 175 && point_y <215){
                    tft.print("4");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 390 && point_x < 470 && point_y > 225 && point_y < 265){
                    tft.print("8");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 390 && point_x < 470 && point_y > 275 && point_y < 315){
                    tft.print("+");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 295 && point_x < 375 && point_y > 125 && point_y <165){
                    tft.print("1");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 295 && point_x < 375 && point_y > 175 && point_y < 215){
                    tft.print("5");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 295 && point_x < 375 && point_y > 225 && point_y < 265){
                    tft.print("9");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 295 && point_x < 375 && point_y > 275 && point_y <315){
                    tft.print("-");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 225 && point_x < 280 && point_y > 125 && point_y <165){
                    tft.print("2");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 225 && point_x < 280 && point_y > 175 && point_y <215){
                    tft.print("6");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 225 && point_x < 280 && point_y > 225 && point_y <265){
                    tft.print(".");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 225 && point_x < 280 && point_y > 275 && point_y <315){
                    tft.print("*");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 105 && point_x < 185 && point_y > 125 && point_y <165){
                    tft.print("3");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 105 && point_x < 185 && point_y > 175 && point_y <215){
                    tft.print("7");
                    delay(1000);
                    ULimitCursorX += 9;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            else if (point_x > 105 && point_x < 185 && point_y > 225 && point_y < 265){
                    tft.print("pi");
                    delay(1000);
                    ULimitCursorX += 18;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
                    ustring.concat("3.14159");
            }
            else if (point_x > 105 && point_x < 185 && point_y > 275 && point_y < 315){
                    tft.print("/");
                    delay(1000);
                    ULimitCursorX += 18;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
            ...
            // Clear button 
            else if (point_x > 10 && point_x < 90 && point_y > 125 && point_y <175){
                    delay(1000);
                    ULimitCursorX = 45;
                    ULimitCursorY = 20;
                    tft.setCursor(ULimitCursorX,ULimitCursorY);
                    ustring = "";
                    tft.fillRect(45,5,315,20,WHITE);
            }
        ...
        while(next_flag != 1){
            ...
            if(touch.z > MINPRESSURE && touch.z < MAXPRESSURE){
                ...
                if (point_x > 10 && point_x < 90 && point_y > 185 && point_y < 245){
                    // Next button gets pressed, we now go to enter the upper limit
                    next_flag = 1;
                }
                else if (point_x > 390 && point_x < 470 && point_y > 125 && point_y <165){
                    tft.print("0");
                ...
                // Clear button
                        ...
                        tft.setCursor(LLimitCursorX,LLimitCursorY);
                        lstring = "";
                        tft.fillRect(25,95,315,20,WHITE);
                }
            } 
        }
    ...
    // Set global variables to lower and upper limit strings that were found here to be sent over to the server
    sLowerlimit = lstring;
    sUpperlimit = ustring;
    // Function used to send upper and lower limit to the server
    Serial.print("UpperL: ");
    ...
    while (go_flag != 1){
        ...
        if(touch.z > MINPRESSURE && touch.z < MAXPRESSURE){
            ...
            //clear button
                ...
                tft.setCursor(cursorX,cursorY);
                innerstring = "";
                tft.fillRect(100,5,380,50,WHITE);
            }
...
/* * * * * * * * * * *  * * * * * * * * * * * * * * * * *
processTouchScreen function takes:
x: starting horizontal point of box for touch
x2: end horizontal point of box for touch
y: starting vertical point of box for touch
y2: end vertical point of box for touch

Links the states for the Main and the Graph display modes.
...
int main(){
    setup();
    // This will keep running using enumeration 
    while (true) {
